...
public:
	...
	static bool DetectObstacle(const SSD::SimPoint3D& vehiclePos, const std::vector<ObstacleStruct>& allObstacles, const HDMapStandalone::MLaneType& laneType,
		const SSD::SimString& laneId, const SSD::SimString& leftNeighborLaneName, SSD::SimPoint3DVector& targetPath, int& warningObstacleIndex)
	{
		...
		double distanceSign = UtilMath::distance(vehiclePos2D, SSD::SimPoint2D(obstacleClosest.pt.x, obstacleClosest.pt.y));
		if (distanceSign < kDistanceSafety)
		{
			...
			SSD::SimPoint3DVector changeLaneBackPath = utilTargetLane::CreateChangeLanePath(changeToPoint, changeBackPoint);
