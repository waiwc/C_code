...
typedef enum AllowedManeuvers {
	...
	AllowedManeuvers_maneuverUTurnAllowed = 1 << 3, //掉头
...
typedef enum ExteriorLights {
	ExteriorLights_unavailable = 1 << 0, //车灯不可用
	ExteriorLights_lowBeamHeadlightsOn = 1<<1,  //近光灯打开
	ExteriorLights_highBeamHeadlightsOn = 1<<2, //远光灯亮
	ExteriorLights_leftTurnSignalOn = 1 << 3, //左转
	ExteriorLights_rightTurnSignalOn = 1 << 4, //右转
	ExteriorLights_hazardSignalOn = 1 << 5, //危险信号开启
	ExteriorLights_automaticLightControlOn = 1 << 6, //自动灯光控制开启
	ExteriorLights_daytimeRunningLightsOn = 1 << 7, //白天行车灯亮
	ExteriorLights_fogLightOn = 1 << 8, //雾灯亮
	ExteriorLights_parkingLightsOn = 1 << 9 //停车灯亮
} e_ExteriorLights;
...
typedef enum VehicleEventFlags {
	VehicleEventFlags_eventHazardLights = 0,//异常车辆
	VehicleEventFlags_eventStopLineViolation = 1,//StopLineViolation，车辆失控
	VehicleEventFlags_eventABSactivated = 2, //紧急制动
	VehicleEventFlags_eventTractionControlLoss = 3, //牵引力控制损失
	VehicleEventFlags_eventStabilityControlactivated = 4, //车身稳定控制系统
	VehicleEventFlags_eventHazardousMaterials = 5, //运输危险物品
	...
	VehicleEventFlags_eventFlatTire = 10, //爆胎
