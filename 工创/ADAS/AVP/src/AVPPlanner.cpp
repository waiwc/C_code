...
void AVPPlanner::planReverseTrajectory()
{
	...
	std::cout << "End point: (" << mParkingEndPoint.x << ", "<< mParkingEndPoint.y << ")" << std::endl;
...
void AVPPlanner::RPplanForwardTrajectory()
{
	...
	if (std::abs(xDiff) > std::abs(yDiff)) {
		...
		for (size_t i = 0; i <= forwardStepCount; ++i)
		{
			...
			mForwardTrajectory.push_back({ x+8.5 , y+2 , 0. });
#if AVP_LOG
...
void AVPPlanner::plan(double type)
{
	planReverseTrajectory();
	if(type == 1)
		RPplanForwardTrajectory();
	else
